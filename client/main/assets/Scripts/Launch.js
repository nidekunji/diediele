...
cc.Class({
    ...
    start () {
        ...
        if (!window.platform) {
            ...
            if (Tools.isFromWx()) {
                WxTools.init(this.afterLogin.bind(this));
            } else {
            ...
            this.afterLogin(window.userInfo);
        }
        ...
        try {
            ...
            if (loginResult) {
                ...
                if (loginResult.userinfo) {
                    ...
                    Prop.set(CONSTS.PROP_TYPE.HP, loginResult.userinfo.health);
                }
                // 注册奖励处理
                ...
                //当前等级
                cc.game.emit('updateCurLevel');
               // Tools.commonChangeHp(-230);
            }

            // 判断是否开启新手指引
           // console.log(loginResult.userinfo, 'loginResult.userinfo');
            //&& !loginResult.userinfo.last_visit_time
                ...
                function showLoginPrices(loginPrizes) {
                    return new Promise(function(resolve, reject) {
                        cc.loader.loadRes("loginprize", function(error, resource){
                            if (error) {
                                console.error("error when loadRes `loginprize`", error);
                                reject();
                                return;
                            }
